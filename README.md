...
Given a 3D mesh, we use [Q-MAT](https://dl.acm.org/doi/10.1145/2753755) to compute the base MAT and the structure MAT for segmentation (Download Q-MAT [here](https://connecthkuhk-my.sharepoint.com/:u:/g/personal/chlin_connect_hku_hk/EQ7qXlADSq5IuseCeCqMPncBbwOoDqRYrTgKEKolrfOUpA?e=KTcaLm)). The Q-MAT simplification is based on vertex number, while we recommend the users to set the vertex number of the base MAT to 2000, and the vertex number of the structure MAT to max{0.001\*initial_MAT_vertex_number, 15}.
