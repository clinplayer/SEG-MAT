...
static void findBasicVariables(node1_t *U, node1_t *V)
{
	...
	while (UfoundNum < _n1 || VfoundNum < _n2)
	{
		...
		if (!found)
		{
			...
			fprintf(stderr, "This typically happens when the EPSILON0 defined in\n");
...
static int isOptimal(node1_t *U, node1_t *V)
{
	...
	return deltaMin >= -EPSILON0 * _maxC;

	/*
	return deltaMin >= -EPSILON0;
	*/
}
	...
	node1_t *UHead)
{
	...
	if (fabs(S[minI] - D[minJ]) <= EPSILON0 * _maxW)  /* DEGENERATE CASE */
	{
		T = S[minI];
		S[minI] = 0;
		D[minJ] -= T;
